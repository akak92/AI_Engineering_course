...
- Capturan los errores de red/autenticación/rate limit y los transforman en
  un `ModelResponse(ok=False, error=...)` en vez de dejarlos escapar.

# TODO: para agregar un proveedor nuevo (ej. Google Gemini, Groq):
# 1. Creá una clase `<Proveedor>Client(BaseLLMClient)` acá, siguiendo el
#    mismo patrón try/except por tipo de error del SDK.
# 2. Agregá el valor correspondiente al enum `Provider` en `schemas.py`.
# 3. Registrala en `AsyncLLMManager._get_client()` (`manager.py`).
"""
...
class OpenAIClient(BaseLLMClient):
    """Adapta el SDK asíncrono de OpenAI a la interfaz común `BaseLLMClient`."""
...
class AnthropicClient(BaseLLMClient):
    """Adapta el SDK asíncrono de Anthropic a la interfaz común `BaseLLMClient`."""
    ...
    def _split_system_and_messages(
        ...
        un mensaje más dentro de la lista. Acá se separan ambas cosas.
        """
