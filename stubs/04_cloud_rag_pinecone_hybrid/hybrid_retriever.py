"""
TEMPLATE: recuperador híbrido (BM25 léxico + Pinecone semántico).

`RAGSystem` encapsula un `EnsembleRetriever` que combina:

- **BM25** (léxico): bueno para términos técnicos exactos y nombres propios
  que un embedding semántico a veces "difumina" al buscar por significado.
...
# Nota de compatibilidad: en versiones recientes del ecosistema LangChain,
# `EnsembleRetriever` vive en el paquete `langchain-classic`, no en
# `langchain-community` (donde vivía antes). Si el import de abajo falla,
# probá `from langchain_community.retrievers import EnsembleRetriever` o
# revisá qué paquete lo expone en tu versión instalada.

Uso:
    system = RAGSystem()
    docs = system.retrieve("tu pregunta acá")
"""
...
# TODO: subí BM25_WEIGHT si tu dominio depende mucho de nombres propios /
# jerga exacta; subí VECTOR_WEIGHT si predominan preguntas parafraseadas.
...
if __name__ == "__main__":
    ...
    for doc in system.retrieve("tu pregunta de ejemplo acá"):
