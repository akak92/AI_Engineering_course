"""
TEMPLATE: pipeline de ingesta a Pinecone.

Fragmenta `data/` (vía `chunking.py`), genera embeddings con OpenAI y los
sube a Pinecone en batches. Guarda el texto original dentro de la metadata
de cada vector (además de la fuente, la categoría y el índice de chunk),
para no depender de una base relacional adicional al recuperar.
...
# Los namespaces separan datos dentro del mismo índice (por entorno, tipo
# de contenido, tenant, etc.); evitan que la búsqueda sea ruidosa/lenta al
# mezclar vectores de dominios distintos en una misma consulta.
